...
const ValuedOnGoing = ({ username }: ValuedOnGoingProps_) => {
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useGetOngoingValuedPosts({
    username,
    size: 10
  })
      ...
      {data?.pages.map((page, pageNo) => {
        ...
        return page.map(({ highestValuer, ...post }, indx) => (
          <div key={`${post._id}_${indx}`} ref={isLastPage && indx === 6 ? nextPageTrigger : undefined}>
            <ValuedPostCard highestValuer={highestValuer._id} {...post} />
          </div>
        ))
      })}
    </div>
...
const OngoingValuedPostScreen = ({ username }: ValuedPostScreenProps_) => {
  return (
...
export default OngoingValuedPostScreen
