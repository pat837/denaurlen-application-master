...
import type { SuccessCallback_, ErrorCallback_ } from '../../../types'
import type { FetchPostsByCategory_ } from '../../../types/category-post.type'

type DeleteCategoryPost_ = {
  categoryId: string
  username: string
...
const useDeleteCategoryPost = (params: DeleteCategoryPost_) => {
  ...
  return useMutation(queries.deletePost, {
    onMutate: (variables): Context_ => {
      ...
      if (categoryPostsData)
        queryClient.setQueryData(categoryKey, () => ({
          ...categoryPostsData,
          data: categoryPostsData.data.filter(post => post._id !== variables.postId)
        }))
...
export default useDeleteCategoryPost
