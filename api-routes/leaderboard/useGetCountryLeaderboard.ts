...
export const countryLeaderboardKey = (countryCode: string) => `jT5aFjBv7i${countryCode}`

const useGetCountryLeaderboard = (countryCode: string, size = 50) => {
  const [key, store] = [countryLeaderboardKey(countryCode), storage.session]
  ...
  return {
    ...useInfiniteQuery(
      key,
      ({ pageParam = 1 }) => leaderboardQueries.getCountryLeaderboard({ countryCode, page: pageParam, size }),
      options
    ),
...
export const useGetCountryLeaderboardWithSearch = (countryCode: string, user: string, size = 20) => {
  const key = [countryLeaderboardKey(countryCode), user]
  ...
  return {
    ...useInfiniteQuery(
      key,
      ({ pageParam = 1 }) =>
        leaderboardQueries.getCountryLeaderboardWithSearch({
          countryCode,
          user,
          page: pageParam,
          size
...
export default useGetCountryLeaderboard
