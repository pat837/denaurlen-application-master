...
export const myCommunityKey = (username: string) => `VXAvaPdMzQ-${username}`

const useGetMyCommunity = ({ username, size = 30, storeData = false }: UseGetCommunityParams_) => {
  const [key, store] = [myCommunityKey(username), storage[storeData ? 'local' : 'session']]
  ...
  return {
    ...useInfiniteQuery(
      key,
      ({ pageParam = 1 }) => profileQueries.myCommunity({ username, size, page: pageParam || 1 }),
      options
    ),
    inValidate: () => {
...
export default useGetMyCommunity
